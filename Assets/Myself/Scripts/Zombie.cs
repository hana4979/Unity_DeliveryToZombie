...
public class Zombie : LivingEntity
{
    ...
    public ParticleSystem deadEffect; // 사망 시 재생할 파티클 효과
    public AudioClip hitSound; // 피격 시 재생할 소리
    public AudioClip deadSound; // 사망 시 재생할 소리

    private Animator zombieAnimator; // 애니메이터 컴포너트
    private AudioSource zombieAudioPlayer; // 오디오 소스 컴포넌트
    //private Renderer zombieRenderer; // 렌더러 컴포넌트

    public float damage = 20f; // 공격력
    public float timeBetdAttack = 0.5f; // 공격 간격
    ...
    public override void Die()
    {
        ...
        // 사망 효과음 재생
        zombieAudioPlayer.PlayOneShot(deadSound);
        // 파티클 효과 재생
        deadEffect.Play();
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if(!dead && Time.time >= lastAttackTime + timeBetdAttack)
        {
